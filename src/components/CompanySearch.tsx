      ...
      <form onSubmit={handleSubmit} className="flex gap-3">
        <div className="relative flex-1">
          ...
          <Input
            ...
            className="pl-8 py-4 text-lg bg-zinc-900/50 backdrop-blur-sm border-zinc-700 focus:border-purple-500 focus:ring-purple-500 rounded-xl text-white placeholder-zinc-400"
            disabled={isLoading}
          />
        </div>
        <Button
      ...
      <div className="mt-4 text-center">
        ...
        <div className="flex flex-wrap justify-center gap-2">
          {["Apple", "Spotify", "Airbnb", "Netflix"].map((company) => (
            <button
              key={company}
              onClick={() => !isLoading && onSearch(company)}
              disabled={isLoading}
